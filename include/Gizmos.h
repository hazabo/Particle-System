...
public:
	...
	static void		create(unsigned int a_maxLines = 65536, unsigned int a_maxTris = 65536);
	static void		destroy();
...
private:
	...
	//Mutex should be added to to all the functions but I will only add it to the ones I use
	std::mutex m_trisMtx; //Used to stop multiple threads changing tri count at the same time
	std::mutex m_transparentTrisMtx;  //used to stop multiple threads changing transparent tri count at the same time
};

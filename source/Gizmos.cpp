...
void Gizmos::create(unsigned int a_maxLines /* = 131072 */, unsigned int a_maxTris /* = 131072 */)
{
...
void Gizmos::addTri(const glm::vec3& a_rv0, const glm::vec3& a_rv1, const glm::vec3& a_rv2, const glm::vec4& a_colour)
{
	if (sm_singleton != nullptr)
	{
		if (a_colour.w == 1)
		{
			...
			{
				sm_singleton->m_trisMtx.lock();
				unsigned int temptricount = sm_singleton->m_triCount;
				sm_singleton->m_triCount++;
				sm_singleton->m_trisMtx.unlock();

				sm_singleton->m_tris[temptricount].v0.position = glm::vec4(a_rv0,1);
				sm_singleton->m_tris[temptricount].v1.position = glm::vec4(a_rv1,1);
				sm_singleton->m_tris[temptricount].v2.position = glm::vec4(a_rv2,1);
				sm_singleton->m_tris[temptricount].v0.colour = a_colour;
				sm_singleton->m_tris[temptricount].v1.colour = a_colour;
				sm_singleton->m_tris[temptricount].v2.colour = a_colour;
			}
		}
		else
		{
			...
			{
				sm_singleton->m_transparentTrisMtx.lock();
				unsigned int temptricount = sm_singleton->m_transparentTriCount;
				sm_singleton->m_transparentTriCount++;
				sm_singleton->m_transparentTrisMtx.unlock();

				sm_singleton->m_transparentTris[temptricount].v0.position = glm::vec4(a_rv0,1);
				sm_singleton->m_transparentTris[temptricount].v1.position = glm::vec4(a_rv1,1);
				sm_singleton->m_transparentTris[temptricount].v2.position = glm::vec4(a_rv2,1);
				sm_singleton->m_transparentTris[temptricount].v0.colour = a_colour;
				sm_singleton->m_transparentTris[temptricount].v1.colour = a_colour;
				sm_singleton->m_transparentTris[temptricount].v2.colour = a_colour;
			}
		}
	}
}
